...
struct tmuxremote_coap_request_node {
    ...
    struct tmuxremote_coap_request_node* next;
};

static void start_listen(struct tmuxremote_coap_handler* handler);
static void request_callback(NabtoDeviceFuture* future, NabtoDeviceError ec,
                             void* userData);
static bool callback_enter(struct tmuxremote_coap_handler* handler);
static void callback_leave(struct tmuxremote_coap_handler* handler);
static bool enqueue_request(struct tmuxremote_coap_handler* handler,
                            NabtoDeviceCoapRequest* request);
...
{
    memset(handler, 0, sizeof(struct tmuxremote_coap_handler));
    ...
    if (handler->future == NULL || handler->listener == NULL) {
        tmuxremote_coap_handler_deinit(handler);
        return NABTO_DEVICE_EC_OUT_OF_MEMORY;
    }
...
{
    tmuxremote_coap_handler_stop(handler);
    ...
    pthread_mutex_lock(&handler->queueMutex);
    struct tmuxremote_coap_request_node* node = handler->queueHead;
    handler->queueHead = NULL;
    handler->queueTail = NULL;
    pthread_mutex_unlock(&handler->queueMutex);

    while (node != NULL) {
        struct tmuxremote_coap_request_node* next = node->next;
...
{
    (void)future;
    struct tmuxremote_coap_handler* handler = userData;
...
static bool enqueue_request(struct tmuxremote_coap_handler* handler,
                            NabtoDeviceCoapRequest* request)
{
    struct tmuxremote_coap_request_node* node =
        (struct tmuxremote_coap_request_node*)calloc(1, sizeof(struct tmuxremote_coap_request_node));
    if (node == NULL) {
        return false;
    }
