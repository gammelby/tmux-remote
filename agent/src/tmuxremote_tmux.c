...
#define NEWLINE "\n"
#define TMUXREMOTE_TMUX_TIMEOUT_MS 2000
#define TMUXREMOTE_TMUX_TERMINATE_GRACE_MS 200
#define TMUXREMOTE_TMUX_LIST_CMDLINE \
...
static void terminate_process(pid_t pid)
{
    ...
    kill(pid, SIGTERM);
    int64_t deadline = monotonic_ms() + TMUXREMOTE_TMUX_TERMINATE_GRACE_MS;
    int status;
...
{
    memset(list, 0, sizeof(struct tmuxremote_tmux_list));
    ...
    char buf[4096];
    ssize_t total = 0;
    int64_t deadline = monotonic_ms() + TMUXREMOTE_TMUX_TIMEOUT_MS;
    ...
    int status;
    if (!waitpid_timeout(pid, TMUXREMOTE_TMUX_TIMEOUT_MS, &status)) {
        return true;
    }
    ...
    while (line != NULL && *line != '\0' &&
           list->count < TMUXREMOTE_TMUX_MAX_SESSIONS) {
...
{
    if (!tmuxremote_tmux_validate_session_name(name)) {
        return false;
    }
